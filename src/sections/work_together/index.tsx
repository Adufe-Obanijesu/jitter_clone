...
export default function WorkTogether() {
  ...
  return (
    <div
      className="mt-[200px] lg:mobile_padding overflow-hidden"
      ref={refs.scope}
    >
      <div className="lg:max-w-[860px] max-w-[400px] mx-auto">
        ...
        <div className="mt-20 overflow-x-hidden lg:overflow-x-visible">
          <div
          ...
          >
            {slider_data.map((item, index) => (
              <div key={item.heading} className="flex">
                {index === 0 && <div className="w-5 rounded-full shrink-0" />}
                <Card {...item} />
                {index !== slider_data.length - 1 && (
                  <div className="w-5 h-full rounded-full" />
                )}
              </div>
            ))}
        ...
        <div className="flex justify-center gap-2 mt-6 lg:hidden">
          {new Array(slider_data.length + 1).fill(0).map((_, i) => (
            <div
              key={i}
              className={cn("w-2 h-2 rounded-full bg-gray-300 cursor-pointer", {
                "bg-gray-500": state.index === i,
              })}
              onClick={() => actions.moveTo(i)}
            />
          ))}
        </div>
        ...
        <div className="hidden lg:flex items-center justify-end gap-1 mt-[50px]">
          <button
            className={cn(
              "w-12 h-12 rounded-full flex items-center justify-center transition_item bg-gray-100 disabled:opacity-40",
              { "bg-gray-50 !cursor-not-allowed": !state.canScrollLeft },
            )}
            onClick={actions.moveLeft}
            disabled={!state.canScrollLeft}
            aria-label={
              state.canScrollLeft ? "Scroll left" : "No more slides to the left"
            }
          >
            <FaChevronLeft />
          </button>
          <button
            className={cn(
              "w-12 h-12 rounded-full flex items-center justify-center transition_item bg-gray-100 disabled:opacity-40",
              { "bg-gray-50 !cursor-not-allowed": !state.canScrollRight },
            )}
